...
impl LeveledCompactionController {
    ...
    pub fn generate_compaction_task(
        ...
        let mut target_level_size = (0..max_level).map(|_| 0).collect::<Vec<_>>();
        let base_level_size_bytes = self.options.base_level_size_mb * 1024 * 1024;

        target_level_size[max_level - 1] =
            real_level_size[max_level - 1].max(base_level_size_bytes);

        let mut base_level = max_level;
        for i in (0..(max_level - 1)).rev() {
