...
impl SsTableIterator {
    ...
    pub fn range_overlap(table: Arc<SsTable>, lower: Bound<Bytes>, upper: Bound<Bytes>) -> bool {
        let table_lower = table.first_key.clone();
        let table_upper = table.last_key.clone();
        match lower {
            Bound::Excluded(bound) => {
                if KeyBytes::for_testing_from_bytes_no_ts(bound) >= table_upper {
                    return false;
                }
            }
        ...
        match upper {
            Bound::Excluded(bound) => {
                if KeyBytes::for_testing_from_bytes_no_ts(bound) <= table_lower {
                    return false;
                }
            }
            Bound::Included(bound) => {
